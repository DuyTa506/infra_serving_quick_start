...
# (bare-metal or cloud VM) with 2x NVIDIA A100 80GB (Ampere).
#
...
echo "=========================================="
echo " vLLM Stack — host setup (2x A100, Docker + NVIDIA)"
echo "=========================================="
...
mkdir -p "$SCRIPT_DIR/../models"     # HF cache dùng chung với stack root
...
==========================================
 ...
 Next (chạy trong thư mục a100/):
   cp .env.example .env      # set API_KEY + HF_TOKEN
   docker compose up -d      # pull + start everything (lần đầu tải model ~18 GB)
   bash status.sh            # health + GPU + container check

 Public endpoints (nginx load-balanced over both A100):
