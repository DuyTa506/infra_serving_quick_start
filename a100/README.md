...
> - **`a100/`** (thư mục này) — 2× A100 80GB Docker (GPTQ INT4)
...
- **GPTQ Int4 chạy tốt trên Ampere.** Nên LLM ở đây là bản **GPTQ INT4**
  của `Qwen3.6-35B-A3B` phục vụ với `--quantization gptq`.

Khác biệt **duy nhất** so với root nằm ở khối `&llm-cmd`:

| | root (RTX 6000 Blackwell) | a100 (A100 Ampere) |
|---|---|---|
| LLM | `nvidia/Qwen3.6-35B-A3B-NVFP4` | `palmfuture/Qwen3.6-35B-A3B-GPTQ-Int4` |
| quant | `modelopt` (NVFP4) | `gptq` (INT4) |
| KV cache | `fp8` | (mặc định fp16/bf16) |
| speculative | MTP | — |
| reasoning parser | `qwen3` (thinking) | — (base instruct, non-thinking) |
...
2 replica → ~2× throughput + HA. Model GPTQ INT4 (~23 GB) thừa sức nằm gọn 1 card nên
**không cần tensor-parallel** kể cả khi A100 có NVLink → zero NCCL cross-GPU.

```
GPU 0 ── llm-0 (GPTQ INT4) ── embedding-0 ── reranker-0  ┐
                                                        ├─► nginx LB ─► clients
GPU 1 ── llm-1 (GPTQ INT4) ── embedding-1 ── reranker-1  ┘
```

| Public port | Service | Model | Endpoint |
|------|---------|-------|----------|
| 8001 | LLM | Qwen3.6-35B-A3B (GPTQ INT4) | `POST /v1/chat/completions` |
...
# 4. Theo dõi
bash status.sh
docker compose logs -f llm-0      # lần đầu tải weights (~23 GB int4)
```

## GPU memory layout (mỗi card 80 GB)

```
LLM  (Qwen3.6-35B-A3B GPTQ INT4, ~23 GB weights + KV)  util 0.80  ≈ 64 GB
...
GPTQ INT4 (~23 GB) chừa rất nhiều VRAM cho KV → có thể nâng `LLM_MAX_MODEL_LEN` (tới 262144) hoặc
...
- **HF cache dùng chung `../models`** → `bge-m3` và reranker không tải lại nếu stack
  root đã pull. Chỉ bản LLM (GPTQ INT4, ~23 GB) là tải mới.
